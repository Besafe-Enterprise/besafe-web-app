...
@alerts_bp.route("/agency/nearby", methods=["GET"])
def agency_nearby():
    """
    Returns agencies sorted by Haversine distance in km from the given lat/lng.
    ...
    for agency in all_agencies:
        ...
        agency_lat, agency_lng = None, None
        distance_km = None
        if isinstance(loc, dict):
            coords = loc.get("coordinates")
            if isinstance(coords, (list, tuple)) and len(coords) >= 2:
                agency_lat, agency_lng = coords[0], coords[1]
                distance_km = haversine_distance(lat, lng, agency_lat, agency_lng)

        agencies.append({
            "id": str(agency["_id"]),
            "name": agency.get("name", ""),
            "phone": agency.get("phone_number", ""),
            "location": {"lat": agency_lat, "lng": agency_lng} if agency_lat is not None else None,
            "distance": round(distance_km, 2) if distance_km is not None else None,
        })

    # Sort agencies by distance ascending (nearest first, agencies with no pin last)
    agencies.sort(key=lambda a: (a["distance"] is None, a["distance"] or float("inf")))
    return jsonify({"agencies": agencies[:limit]})
